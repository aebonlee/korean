...
function Settings() {
  ...
  const handleSaveApiKey = () => {
    if (!apiKey.trim()) {
      setSaveMessage(String(t('API 키를 입력해주세요.', 'Please enter your API key.')));
      return;
    }

    if (!apiKey.trim().startsWith('sk-')) {
      setSaveMessage(String(t('올바른 OpenAI API 키 형식이 아닙니다. (sk-로 시작해야 합니다)', 'Invalid OpenAI API key format. (Must start with sk-)')));
      return;
    }
    ...
    try {
      ...
      setApiKeyStatus('registered');
      setSaveMessage(String(t('API 키가 저장되었습니다.', 'API key saved.')));
      setTimeout(() => setSaveMessage(''), 3000);
    } catch {
      setSaveMessage(String(t('API 키 저장에 실패했습니다.', 'Failed to save API key.')));
    }
  };
  ...
  const handleDeleteApiKey = () => {
    try {
      ...
      setApiKeyStatus('not_registered');
      setShowApiKey(false);
      setSaveMessage(String(t('API 키가 삭제되었습니다.', 'API key deleted.')));
      setTimeout(() => setSaveMessage(''), 3000);
    } catch {
      setSaveMessage(String(t('API 키 삭제에 실패했습니다.', 'Failed to delete API key.')));
    }
  };
  ...
  return (
    <>
      <SEOHead
        title={String(t('설정 - Korean Pro', 'Settings - Korean Pro'))}
        description={String(t('Korean Pro 설정. API 키, 테마, 언어를 관리하세요.', 'Korean Pro settings. Manage your API key, theme, and language.'))}
      />
