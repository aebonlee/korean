...
export default function VocabHome() {
  ...
  return (
    <>
      <SEOHead title={String(t('필수 한국어 어휘 - Korean Pro', 'Essential Korean Vocabulary - Korean Pro'))} description={String(t(`기초, 일상, 비즈니스, TOPIK 필수 한국어 어휘 ${TOTAL}개를 학습하세요.`, `Study ${TOTAL} essential Korean vocabulary across basic, daily, business, and TOPIK categories.`))} />
            ...
            {categories.map((cat, index) => (
              <Link to={cat.path} key={cat.path} className={`topic-card card topic-card--${cat.level}`} data-aos="fade-up" data-aos-delay={index * 100}>
                ...
                <div className="topic-card__meta">
                  <span className="topic-card__level"><i className="fas fa-signal"></i> {t((LEVEL_LABELS as any)[cat.level].ko, (LEVEL_LABELS as any)[cat.level].en)}</span>
