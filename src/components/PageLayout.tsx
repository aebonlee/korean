...
export interface Section { id: string; ko: string; en: string; }

interface PageLayoutProps { sections?: Section[]; category?: string; children?: React.ReactNode; }

export default function PageLayout({ sections = [], category, children }: PageLayoutProps) {
  const { t } = useLanguage();
  ...
  const items = category ? navData[category as keyof typeof navData] : null;
  const catTitle = category ? categoryTitles[category as keyof typeof categoryTitles] : null;
  ...
  useEffect(() => {
    ...
    root.querySelectorAll('.expression-table tbody td:first-child strong').forEach((el: Element) => {
    ...
    // dialogue p elements
    root.querySelectorAll('.dialogue p').forEach((p: Element) => {
    ...
    return () => {
      root.querySelectorAll('.tts-play-btn').forEach((btn: Element) => btn.remove());
    };
  }, []);
