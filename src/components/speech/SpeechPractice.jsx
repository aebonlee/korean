...
export default function SpeechPractice({ targetText, targetMeaning }) {
  const { t } = useLanguage();
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'no-speech') {
        setError(t('음성이 감지되지 않았습니다. 다시 시도해주세요.', 'No speech detected. Please try again.'));
      } else {
        setError(t(`오류: ${event.error}`, `Error: ${event.error}`));
      }
    };
    ...
    <div className="speech-practice">
      ...
      <div className="speech-controls">
        <button
          ...
          aria-label={isListening ? t('녹음 중지', 'Stop recording') : t('녹음 시작', 'Start recording')}
        >
          {isListening ? (
